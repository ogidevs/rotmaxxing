...
const HomePage: React.FC = () => {
    ...
    return (
        <div className='flex flex-col items-center justify-center h-screen text-center'>
            ...
            <textarea onBlur={(event) => setText((event.target as HTMLTextAreaElement).value)} placeholder="Type here..." className='w-1/2 h-1/2 p-4 m-4 border border-gray-300 rounded-lg text-white bg-gray-800'></textarea>
            {videoUrl && <iframe src={videoUrl} width="560" height="315" frameBorder="0" allowFullScreen></iframe>}
            <Button onClick={handleClick} className='m-4' disabled={loading}>Generate Brain Rot</Button>
            <Button onClick={logout} className='m-4'>Logout</Button>
            {loading && <p>Loading...</p>}
        </div>
    );
};
